...
//#define WIFI_SSID "Rmic2122"
//#define WIFI_PASSWORD "Rmic2122%$"
//#define WIFI_SSID "IotDemo"
//#define WIFI_PASSWORD ""
#define WIFI_SSID "AndroidAPAG"
#define WIFI_PASSWORD "mdrq6475"
 ...
 *
 */
#define API_KEY "AIzaSyAM5T2Gcq0CIZxUgMjVY08popnff-YvpNE"

/* 3. If work with RTDB, define the RTDB URL */
#define DATABASE_URL "https://iot-alarm-app.firebaseio.com/"  //<databaseName>.firebaseio.com or <databaseName>.<region>.firebasedatabase.app
...
void setup()
{
  ...
  // These lines bridge the two logical serial paths on the C6
...
if (millis() - dataMillis > 5000 && signupOK && Firebase.ready())
  {
    ...
    dataMillis = millis(); 
    path1="alarm_status";
    path2 = "enable_alert_sound";
    path3 = "measurement";

    if(getdistancelvl() == 1 ){
          Serial.println("Distance ");
          measurement++;
          Serial.printf("Set bool... %s\n", Firebase.RTDB.setBool(&fbdo, path1, true) ? "ok" : fbdo.errorReason().c_str());
          Serial.printf("Set int... one more measurement increase %s\n", Firebase.RTDB.setInt(&fbdo, path3 , measurement) ? "ok" : fbdo.errorReason().c_str());
          digitalWrite(LED_BUILTIN, HIGH);
          Serial.println("LED ON");
          Firebase.RTDB.getString(&fbdo,path2);
          result= fbdo.to<String>();
 
          if ( result == "true"){
              Serial.println("Enable alert sound is True");
              tone(BUZZER_PIN,450); 
              delay(1000);
              noTone(BUZZER_PIN);
        } else {
              Serial.println("Enable alert is false");
              delay(1000);
              noTone(BUZZER_PIN);
          }
        } else { /* Means is low*/
            Serial.println("movement deactivated");
            Serial.printf("Set bool... alarm status is now false %s\n", Firebase.RTDB.setBool(&fbdo, path1, false) ? "ok" : fbdo.errorReason().c_str());
            digitalWrite(LED_BUILTIN, LOW);
          
        }
        Serial.println("Final one second delay before a new loop");
        delay(1000);
